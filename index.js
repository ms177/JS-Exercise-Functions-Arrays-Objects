...
function makePersonObject(id, name, email) {
  let newPerson = {id: id, name: name, email: email};
  return newPerson;
}
...
function getName(obj) {
  let result = `Hello, my name is ${obj.name}`;
  return result
}
...
function makeSmartPerson(name) {
  return {name: name, sum: (num1, num2) => num1 + num2, speak: () => `Hello, my name is ${name}`}
}
...
function getCarInfoById(inventory,id) {
  ...
  for (let i in inventory) {
    if (inventory[i].id === id) findCar = inventory[i];
  }

  return `This car is a ${findCar.car_make} ${findCar.car_model}`;
}
...
function getOlderCars(inventory,max_year) {
  ...
  for ( i in inventory) {
    let car = inventory[i];
    if (car.car_year <= max_year) oldCar.push(car);
  }
...
function getGermanCars(inventory) {
  ...
  for ( i in inventory) {
    let car = inventory[i];
    if (car.car_make == `Audi` || car.car_make == `Mercedes-Benz` || car.car_make == `Volkswagen` || car.car_make == `BMW`  ) germanCar.push(car);
  }
...
function carMaker(odometer) {
  return {odometer: odometer, 
          drive(distance) { this.odometer += distance; return this.odometer }
  }
}
